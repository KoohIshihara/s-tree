...
var exportScenarioAsECA = function(){

  // テンプレートを全て作成する
  for(var i=0; i<scenarioGetSize(scenarioArray); i++){
    var content = scenarioGetContentByIndex(scenarioArray, i);
  ...
  var projectId = riot.currentProject.id;
  var startPointEvent = scenarioGetContent(scenarioArray, `start-point-${projectId}`);
  var firstEventId = startPointEvent.next;

  var firstEvent = scenarioGetContent(scenarioArray, firstEventId);
  var firstEventRule = '';
  ...
  if(currentEvent.id.indexOf('goTo')>=0){
    var gotoEvent = scenarioGetContent(scenarioArray, currentEvent.id);
    bodyEventOfFirst += `  DO send_message(\"${gotoEvent.toId}\")`;
    var currentEvent = scenarioGetContent(scenarioArray, gotoEvent.toId);
  }else{
  ...
  while(true){
    ...
    if(nextId.indexOf('goTo')>=0){
      ...
      var gotoEvent = scenarioGetContent(scenarioArray, currentEvent.next);
      bodyEventOfFirst += ` THEN send_message(\"${gotoEvent.toId}\")`;
      currentEvent = scenarioGetContent(scenarioArray, gotoEvent.toId);
    }else{
      bodyEventOfFirst += ` THEN send_message(\"${currentEvent.next}\")`;
      currentEvent = scenarioGetContent(scenarioArray, nextId);
    }
    //bodyEventOfFirst += ` THEN send_message(\"${nextId}\")`;

    if(currentEvent.type == 'goto') currentEvent = scenarioGetContent(scenarioArray, currentEvent.toId);
  }
  ...
  END
  */
  var allSelectionNodes = scenarioGetAllSelectionNode(scenarioArray);
  for(var i=0; i<allSelectionNodes.length; i++){
    ...
    for(var j=0; j<selections.length; j++){
      ...
      if(selections[j].next.indexOf('goTo')>=0){
        var gotoEvent = scenarioGetContent(scenarioArray, selections[j].next);
        selectionsRule += `DO send_message(\"${gotoEvent.toId}\")`;
        var currentEvent = scenarioGetContent(scenarioArray, gotoEvent.toId);
      }else{
        selectionsRule += `DO send_message(\"${selections[j].next}\")`;
        var currentEvent = scenarioGetContent(scenarioArray, selections[j].next);
      }
      ...
      while(true) {
        ...
        if(currentEvent.next.indexOf('goTo')>=0){
          ...
          currentEvent = scenarioGetContent(scenarioArray, currentEvent.next);
        }
  ...
  END
  */
  var allOpenQuestionNodes = scenarioGetOpenQuestionNode(scenarioArray);
  for(var i=0; i<allOpenQuestionNodes.length; i++){
    ...
    if(allOpenQuestionNodes[i].next.indexOf('goTo')>=0){
      var gotoEvent = scenarioGetContent(scenarioArray, allOpenQuestionNodes[i].next);
      openquestionRule += `DO send_message(\"${gotoEvent.toId}\")`;
      var currentEvent = scenarioGetContent(scenarioArray, gotoEvent.toId);
    }else{
      openquestionRule += `DO send_message(\"${allOpenQuestionNodes[i].next}\")`;
      var currentEvent = scenarioGetContent(scenarioArray, allOpenQuestionNodes[i].next);
    }
    ...
    while(true){
      ...
      if(currentEvent.next.indexOf('goTo')>=0){
        ...
        var currentEvent = scenarioGetContent(scenarioArray, currentEvent.next);
      }
  ...
  download(`${riot.currentProject.title}.ecal`, resultText)
